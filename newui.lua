...
do
    ...
    function library.fovcircle:update_position()
        self:set('Position', self._mode == 'center' and inputservice:GetMouseLocation() or library.screensize / 2)
    end

    function library.fovcircle:update_radius()
        self._fov = library.flags[self._flag .. '_fov_radius'] + (library.flags[self._flag .. '_fov_dynamic'] and -(library.camera.FieldOfView * 2) or 0)
    ...
    function library.fovcircle:update()
        ...
        self:set('Color', library.flags[self._flag .. '_fov_color'], true)
        self:set('Visible', library.flags[self._flag .. '_fov_enabled'])
        self:set('NumSides', library.flags[self._flag .. '_fov_sides'])
        ...
        function library.meta.colorpicker:update()
            ...
            if self.selected ~= nil then
                ...
                if self.selected.flag ~= nil then
                    library.flags[self.selected.flag] = self.color
                end
            end
    ...
    function library.meta.options.label:get_text()
        return self.objects.label.Text
    end
    ...
    do
        ...
        function library.meta.options.toggle:new(properties)
            ...
            toggle.state = properties.state and properties.state or false
    ...
    do
        ...
        function library.meta.options.slider:new(properties)
            ...
            slider.parent = self
            slider.value = properties.value or 0
    ...
    do
        ...
        function library.meta.options.colorpicker:new(properties)
            ...
            if properties.flag ~= nil then
                library.flags[properties.flag] = colorpicker.color
            end
    ...
    do
        ...
        function library.meta.options.keybind:new(properties)
            ...
            keybind.state = properties.state or false
            keybind.mode = properties.mode or 'toggle'
            keybind.key = properties.default or 'none'

            if properties.indicator then
                keybind.indicator = library.keybind_indicator:value({key = keybind.text == '' and keybind.flag or keybind.text, enabled = true})
            end
...
function library:CreateSettings(Window)
    ...
    Sections["Settings"]["Config"]:button({text = "Save", confirm = true, callback = function()
        xpcall(function()
            if library.flags.Config_Selected ~= "" or library.flags.Config_Selected ~= "none" then
                ...
                library:notification(("Successfully saved config '%s'"):format(library.flags.Config_Selected), 5, Color3.new(0.35, 1, 0.35))
            end
        end, function(err)
    ...
    Sections["Settings"]["Theme"]:button({text = "Save", confirm = true, callback = function()
        ...
        xpcall(function()
            ...
            library:notification(("Successfully saved theme '%s'"):format(library.flags.Theme_Selected), 5, Color3.new(0.35, 1, 0.35))
        end, function(err)
