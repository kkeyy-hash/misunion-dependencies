...
local library = {
    ...
    signal      = loadstring(game:HttpGetAsync('https://raw.githubusercontent.com/kkeyy-hash/nordhook/main/signal.lua'))(),
    stat        = {fps = 0, ping = 0},
    drawings    = {
        active    = {},
        draggable = {},
...
do
    ...
    function library:notification(message, duration, color)
        ...
        if color ~= nil then
            ...
            notification.objects.accent.Color = color and color or library.theme.Accent
            notification.objects.progress.Theme = {}
            notification.objects.progress.Color = color and color or library.themes.Accent
        end
        ...
        function notification:remove()
            if library and library.notifs then table_remove(library.notifs, table_find(library.notifs, notification)) end
    ...
    warn(tostring(library.gamename))
    if not isfolder("nordhook/configs/" .. tostring(library.gamename)) then makefolder("nordhook/configs/" .. tostring(library.gamename)) end

    local Request = (syn and syn.request) or (http_request and http_request) or request
    ...
    function library:load_config(config)
        ...
        if typeof(config) == 'string' then
            local path = library.cheatname .. '/configs/' .. tostring(library.gamename) .. "/" .. config .. '.json'
        ...
        for flag, value in next, config do
            xpcall(function()
                ...
                elseif option.class == 'dropdown' and type(value) == "table" then
                    table.foreach(value, function(index, val)
                        if val == true then
                            option:select(value)
                        end
                    end)
                ...
                library:notification(("unable to set '%s' to '%s' [%s]"):format(flag, typeof(value), tostring(err)), 5, Color3.new(1, 0.35, 0.35))
            end)
        end
    ...
    function library:save_config(name, existscheck)
        local path = 'nordhook/configs/' .. tostring(library.gamename) .. "/" .. name .. '.json'
        warn(path)
        if existscheck then
        ...
        print(http:JSONEncode(config))
        writefile(path, http:JSONEncode(config))
    end
    function library:LoadAutoloadConfig()
		if isfile(library.cheatname .. "/configs/" .. tostring(library.gamename) .. "/autoload.txt") then
			local name = readfile(library.cheatname .. "/configs/"  .. tostring(library.gamename) .. "/autoload.txt")

			local success, err = library:load_config(name)
			if success ~= nil and success == false then
                return library:notification(string.format("failed to load autoload config '%s'", name or "ERROR"), 5, Color3.new(1, 0.35, 0.35))
			end

			library:notification(string.format("successfully auto loaded config '%s'", name), 5)
		end
	end
    ...
    function library:save_theme(name, existscheck)
        local path = library.cheatname .. '/themes/' .. name .. '.json'
        if existscheck then
    ...
    function library:LoadAutoloadTheme()
		if isfile(library.cheatname .. "/themes/autoload.txt") then
			...
			if success ~= nil and success == false then
                return library:notification(string.format("failed to load autoload theme '%s'", name or "ERROR"), 5, Color3.new(1, 0.35, 0.35))
			end

			library:notification(string.format("successfully auto loaded theme '%s'", name), 5)
		end
	end
...
do
    ...
    do
        ...
        function library.meta.options.dropdown:new(properties)
            ...
            dropdown.values = {}
            dropdown.selected = properties.multi and {} or properties.default and properties.default or ''
...
library.watermark = library:create('watermark', {text = {"nordhook.cc", "uid " .. tostring(userData.uid), scriptData.build .. " build", tostring(library.gamename), "999ms", "999 fps"}, enabled = true})
...
function library:create_settings(Window)
    local Tabs = {
    ...
    Sections["settings"]["main"]:slider({default = 1, min = 0, max = 100, increment = 0.1, text = "position x", flag = "keybind_indicator_x", enabled = true, callback = function(value)
    ...
    Sections["settings"]["main"]:slider({default = 45, min = 0, max = 100, increment = 0.1, text = "position y", flag = "keybind_indicator_y", enabled = true, callback = function(value)
        library.keybind_indicator.set_position(UDim2.new(library.flags.keybind_indicator_x / 100, 0, library.flags.keybind_indicator_y / 100, 0))
    end})
    ...
    Sections["settings"]["main"]:dropdown({text = "position", flag = "watermark_position", values = {"top right", "top left", "bottom right", "bottom left", "top"}, default = "top", callback = function(value)
    ...
    Sections["settings"]["config"]:button({text = "create", confirm = true, callback = function()
        xpcall(function()
            ...
            library:notification(("successfully created config '%s'"):format(library.flags.Config_Name), 5)
        end, function(err)
    ...
    Sections["settings"]["config"]:button({text = "save", confirm = true, callback = function()
        xpcall(function()
            if library.flags.Config_Selected ~= "" or library.flags.Config_Selected ~= "none" then
                local selectedcfg = library.flags.Config_Selected
                library:save_config(selectedcfg)
                library:notification(("successfully saved config '%s'"):format(selectedcfg), 5)
            end
        end, function(err)
    ...
    Sections["settings"]["config"]:button({text = "load", confirm = true, callback = function()
        xpcall(function()
            if library.flags.Config_Selected ~= "" or library.flags.Config_Selected ~= "none" then
                local selectedcfg = library.flags.Config_Selected
                library:load_config(selectedcfg)
                library:notification(("successfully loaded config '%s'"):format(selectedcfg), 5)
            end
        end, function(err)
    ...
    Sections["settings"]["config"]:button({text = "set as auto load config", confirm = true, callback = function()
        xpcall(function()
            if library.options.autoload_cfg and library.flags.Config_Selected ~= "" or library.flags.Config_Selected ~= "none" then
                writefile(library.cheatname .. "/configs/" .. tostring(library.gamename) .. "/autoload.txt", library.flags.Config_Selected)
                ...
                library:notification(("successfully set auto load config to '%s'"):format(library.flags.Config_Selected), 5)
            end
        end, function(err)
            library:notification(err or ("unable to load config '%s'"):format(library.flags.Config_Selected), 5, Color3.new(1, 0.35, 0.35))
        end)
    end})
    local configname = "none"
    if isfile(library.cheatname .. "/configs/" .. tostring(library.gamename) .. "/autoload.txt") then
        configname = readfile(library.cheatname .. "/configs/" .. tostring(library.gamename) .. "/autoload.txt")
    end
    Sections["settings"]["config"]:label({text = "auto load: " .. configname, flag = "autoload_cfg"})

    if isfolder(library.cheatname .. "/configs/" .. tostring(library.gamename)) then
        for i,v in next, listfiles(library.cheatname .. "/configs/" .. tostring(library.gamename)) do
    ...
    Sections["settings"]["theme"]:button({text = "create", confirm = true, callback = function()
        xpcall(function()
            ...
            library:notification(("successfully created theme '%s'"):format(library.flags.Theme_Name), 5)
        end, function(err)
    ...
    Sections["settings"]["theme"]:button({text = "save", confirm = true, callback = function()
        ...
        xpcall(function()
            ...
            library:notification(("successfully saved theme '%s'"):format(library.flags.Theme_Selected), 5)
        end, function(err)
    ...
    Sections["settings"]["theme"]:button({text = "load", confirm = true, callback = function()
        xpcall(function()
            ...
            library:notification(("successfully loaded theme '%s'"):format(library.flags.Theme_Selected), 5)
        end, function(err)
    ...
    Sections["settings"]["theme"]:button({text = "set as auto load theme", confirm = true, callback = function()
        xpcall(function()
            if library.options.autoload_theme and library.flags.Theme_Selected ~= "" or library.flags.Theme_Selected ~= "none" then
                ...
                library:notification(("successfully set auto load theme to '%s'"):format(library.flags.Theme_Selected), 5)
            end
        end, function(err)
            library:notification(err or ("unable to load theme '%s'"):format(library.flags.Theme_Selected), 5, Color3.new(1, 0.35, 0.35))
        end)
