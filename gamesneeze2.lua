    ...
    LPH_NO_VIRTUALIZE(function()
        ...
        library = {
            ...
            folders = {
                ...
                themes = "nordhook/themes",
                sounds = "nordhook/sounds"
            },
            shared = {
                initialized = false,
                fps = 0,
        ...
        do
            ...
            function library:Loader(info)
                ...
                library.began[#library.began + 1] = function(Input)
                    ...
                    if window.currentContent.textbox then
                        if Find(utility.Keyboard.Letters, utility:InputToString(Input.KeyCode)) and not window.currentContent.textbox.numbers then
                        ...
                        elseif utility:InputToString(Input.KeyCode) == "Space" and not window.currentContent.textbox.numbers then
                            window.currentContent.textbox.Fire(" ")
                        elseif utility.Keyboard.Modifiers[utility:InputToString(Input.KeyCode)] then
                            if window.currentContent.textbox.numbers then
                                if utility.Keyboard.Numbers[Input.KeyCode.Name] then
                                    window.currentContent.textbox.Fire((utility:InputToString(Input.KeyCode)):upper())
                                end
                            else
                                if uis:IsKeyDown(Enum.KeyCode.LeftShift) then
                ...
                function playerList:Refresh(Relation)
                    ...
                    if Selection then
                        if lastselection ~= Selection then
                            ...
                            --
                            local response = game:HttpGet(string.format("https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=%d&size=150x150&format=Png&isCircular=false", Selection[1].UserId))
                            local data = hs:JSONDecode(response).data
            ...
            function sections:Toggle(info)
                ...
                function toggle:Keybind(info)
                    ...
                    --
                    local allowedKeyCodes = {"Q","W","E","R","T","Y","U","I","O","P","A","S","D","F","G","H","J","K","L","Z","X","C","V","B","N","M","One","Two","Three","Four","Five","Six","Seveen","Eight","Nine","Zero", "Minus", "Equals","F1","F2","F3","F4","F5","F6","F7","F8","F9","F10","F11","F12","Insert","Tab","Home","End","LeftAlt","Hash","LeftControl","LeftShift","RightAlt","RightControl","RightShift","CapsLock","Space"}
                    local allowedInputTypes = {"MouseButton1","MouseButton2","MouseButton3"}
                    local shortenedInputs = {["MouseButton1"] = "MB1", ["MouseButton2"] = "MB2", ["MouseButton3"] = "MB3", ["Hash"] = "#", ["Insert"] = "Ins", ["Minus"] = "-", ["Equals"] = "=", ["LeftAlt"] = "LAlt", ["LeftControl"] = "LC", ["LeftShift"] = "LS", ["RightAlt"] = "RAlt", ["RightControl"] = "RC", ["RightShift"] = "RS", ["CapsLock"] = "Caps"}
                    --
            ...
            function sections:TextBox(info)
                ...
                local numbers = info.numbers or info.Numbers or false
                local callback = info.callback or info.callBack or info.Callback or info.CallBack or function()end
            ...
            function sections:Keybind(info)
                ...
                --
                local allowedKeyCodes = {"Q","W","E","R","T","Y","U","I","O","P","A","S","D","F","G","H","J","K","L","Z","X","C","V","B","N","M","Hash","One","Two","Three","Four","Five","Six","Seveen","Eight","Nine","Zero", "Minus", "Equals","F1","F2","F3","F4","F5","F6","F7","F8","F9","F10","F11","F12","Insert","Tab","Home","End","LeftAlt","LeftControl","LeftShift","RightAlt","RightControl","RightShift","CapsLock","Space"}
                local allowedInputTypes = {"MouseButton1","MouseButton2","MouseButton3"}
                local shortenedInputs = {["MouseButton1"] = "MB1", ["MouseButton2"] = "MB2", ["MouseButton3"] = "MB3", ["Hash"] = "#", ["Insert"] = "Ins", ["LeftAlt"] = "LAlt", ["LeftControl"] = "LC", ["LeftShift"] = "LS", ["RightAlt"] = "RAlt", ["RightControl"] = "RC", ["RightShift"] = "RS", ["CapsLock"] = "Caps"}
                --
                local keybind_outline = utility:Create("Frame", {Vector2.new(section.section_frame.Size.X-(40+4),keybind.axis), section.section_frame}, {
                    Size = utility:Size(0, 40, 0, 17),
                    Position = utility:Position(1, -(40+4), 0, keybind.axis, section.section_frame),
            ...
            function sections:Colorpicker(info)
                ...
                function colorpicker:Set(color, transp_val)
                    if typeof(color) == "table" and rawget(color, "Color") then
                        ...
                        colorpicker_frame.Color = Color3.fromHSV(colorpicker.current[1], colorpicker.current[2], colorpicker.current[3])
                        colorpicker_frame.Transparency = 1 - color.Transparency
                        callback(Color3.fromHSV(colorpicker.current[1], colorpicker.current[2], colorpicker.current[3]), colorpicker.current[4])
                    elseif typeof(color) == "table" and not rawget(color, "Color") then
                        colorpicker_frame.Color = Color3.fromHSV(colorpicker.current[1], colorpicker.current[2], colorpicker.current[3])
                        colorpicker_frame.Transparency = 1 - colorpicker.current[4]
                        callback(Color3.fromHSV(colorpicker.current[1], colorpicker.current[2], colorpicker.current[3]), colorpicker.current[4])
                    elseif typeof(color) == "Color3" then
...
local NordHook = {
    ...
    Account = {
        UserData = getgenv().userData or {uid = "1"},
        ScriptData = getgenv().scriptData or {build = "developer"}
    },
    Configs = {},
    Debug = false
}

function library.utility:UpdatePreview(Pass)
...
library.utility:Connection(rs.Heartbeat, function()
    library.utility:ThreadFunction(library.UpdateHue, "0x01")
    local Tick = tick()

    if (Tick - NordHook.Locals.LastPreviewUpdate) > 0.05 then
        library.utility:ThreadFunction(utility.UpdatePreview)
...
local Library, Utility, Flags, Theme = library, library.utility, library.pointers, theme
...
do
    function library.utility:GetTableIndexes(Table, Custom)
        local Table2 = {}
    ...
    function library.utility:ConvertTable(Table1)
        local Table2 = {}
    ...
    function library.utility:ConvertString(Value)
    ...
    function library.utility:Encode(Table)
        local Table2 = {}

        for Index, Value in pairs(Table) do
            Table2[Index] = library.utility:ConvertString(Value)
        end

        return HttpService:JSONEncode(Table2)
    end

    function library.utility:Decode(Table)
    ...
    function library:PerformConfigAction(ConfigName, Action)
        ...
        if ConfigName then
            if Action == "Delete" then
                ...
                if Found then
                    delfile(("nordhook/configs/%s"):format(ConfigName .. ".json"), Config)
            ...
            elseif Action == "Save" then
                ...
                if Config then
                    if not Find(NordHook.Configs, ConfigName) then
                        writefile(("nordhook/configs/%s"):format(ConfigName .. ".json"), Config)
            ...
            elseif Action == "Load" then
                local Config = readfile(("nordhook/configs/%s"):format(ConfigName .. ".json"))
    ...
    function library.utility:ThreadFunction(Func, Name, ...)
        local Func = Name and function()
            local Passed, Statement = pcall(Func)

            if not Passed and NordHook.Debug then
                warn("NordHook:\n", "              " .. Name .. ":", Statement)
            end
        end or Func
